 ...
 * Butterworth BPF and BSF are made by cascading (BPF) or paralleling (BSF) a
 * Butterworth LPF and Butterworth HPF.
 */
#pragma once
...
public:
  tp_coeffs &calculate_coeffs(float bw, int fc, int fs) {
    coef_size_t c = 1.0 / (tan(pi<coef_size_t> * fc * bw / fs));
    coef_size_t d = 2.0 * cos(2.0 * pi<coef_size_t> * fc / fs);
