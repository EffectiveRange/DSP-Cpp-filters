 ...
 * filters are placed in parallel with the same cutoff frequency, their outputs
 * sum exactly and the resulting response is flat. They are often used in
 * crossovers.
 */
#pragma once
...
public:
  tp_coeffs &calculate_coeffs(int fc, int fs) {
    coef_size_t th = pi<coef_size_t> * fc / fs;
    coef_size_t Wc = pi<coef_size_t> * fc;
