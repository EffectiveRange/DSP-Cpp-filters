 ...
 * Shelving filters are used in many tone controls, especially when there are
 * only two, bass and treble, which are almost always implemented as shelf
 * types. The filters have a corner frequency and gain or attenuation value.
 */
#pragma once
...
public:
  tp_coeffs &calculate_coeffs(float gain_db, int fc, int fs) {
    coef_size_t th = 2.0 * pi<coef_size_t> * fc / fs;
